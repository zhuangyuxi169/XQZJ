...
<body style="background-color: #E0F0F8;">
	...
	<form id="form1" method="post">
		...
		<c:if test="${infoBean.id==''}">
		<center>
			<button style="width:100px ;height:30px" formaction="${pageContext.request.contextPath }/XuqiuServlet?method=add&username=${userBean.name }&status=0" >保存</button>
			<button style="width:100px ;height:30px" formaction="${pageContext.request.contextPath }/XuqiuServlet?method=add&username=${userBean.name }&status=1" >提交</button>
		</center>
		</c:if>	
		
		<c:if test="${infoBean.id!=''}">
		<center>
			<button style="width:100px ;height:30px" formaction="${pageContext.request.contextPath }/XuqiuServlet?method=update&id=${infoBean.id }&status=0" >保存修改</button>
			<button style="width:100px ;height:30px" formaction="${pageContext.request.contextPath }/XuqiuServlet?method=update&id=${infoBean.id }&status=1" >提交</button>
			<button style="width:100px ;height:30px" type="button" onclick="location='${pageContext.request.contextPath }/XuqiuServlet?method=list&username=${userBean.name }&type=${param.type}'" >返回</button>
		</center>
		</c:if
		>			
	</form>
