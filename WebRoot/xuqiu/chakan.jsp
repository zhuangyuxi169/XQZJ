...
<body style="background-color: #E0F0F8;">
	...
	<form  id="form1" method="post">
		...
		<c:if test="${infoBean.status!=0}">
		<center>
			<button type="button" style="width:100px ;height:30px"
				onclick="location='${pageContext.request.contextPath }/xuqiu/list?username=${userBean.name }&juese=${userBean.juesebean.name }&type=${param.type}'">返回</button>
		</center>
		</c:if>
		<c:if test="${infoBean.id>0&&infoBean.status==0 }">
		<center>
			<button type="button" style="width:100px ;height:30px"
				onclick="location='${pageContext.request.contextPath }/xuqiu/update?id=${infoBean.id }&status=0'">保存修改</button>
			<button type="button" style="width:100px ;height:30px"
				onclick="location='${pageContext.request.contextPath }/xuqiu/update?id=${infoBean.id }&status=1'">提交</button>
		
		</center>
		</c:if>
	
		<c:if test="${empty infoBean.id||infoBean.id==-1}">
		<center>
			<button type="button" style="width:100px ;height:30px"
				onclick="location='${pageContext.request.contextPath }/xuqiu/add?status=0'">确认保存</button>
			<button type="button" style="width:100px ;height:30px"
				onclick="location='${pageContext.request.contextPath }/xuqiu/add?status=1'">确认提交</button>
